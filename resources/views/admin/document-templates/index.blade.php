<x-layouts.app :title="__('Шаблоны')">
    ...
    <div class="flex flex-col flex-1 w-full h-full gap-6">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold">Все Шаблоны</h1>
            <flux:button href="{{ route('admin.document-templates.create') }}" icon="plus" variant="primary">
                Создать шаблон
            </flux:button>
        </div>
    <div class="hidden sm:block border rounded-xl border-gray-200">
    ...
    <div class="sm:hidden space-y-2">
        @foreach($documents as $template)
            <div class="flex items-center justify-between p-4 bg-white dark:bg-zinc-800 rounded-xl shadow-sm hover:shadow-md transition">
                <div class="flex items-center gap-3">
                    <span class="flex items-center justify-center w-8 h-8">
                        <x-icon.template-icon />
                    </span>
                    <a href="{{ route('admin.document-templates.show', $template->id) }}" class="font-medium text-gray-900 dark:text-gray-100 hover:text-gray-700 transition">
                        {{ $template->name }}
                    </a>
                </div>

                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-zinc-700 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5 text-gray-500 dark:text-gray-300">
                            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                  d="M12 5.92A.96.96 0 1 0 12 4a.96.96 0 0 0 0 1.92m0 7.04a.96.96 0 1 0 0-1.92a.96.96 0 0 0 0 1.92M12 20a.96.96 0 1 0 0-1.92a.96.96 0 0 0 0 1.92"/>
                        </svg>
                    </button>

                    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-40 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg shadow-lg z-50">
                        <a href="{{ route('admin.document-templates.show', $template->id) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-zinc-700">Просмотр</a>
                        <a href="{{ route('admin.document-templates.edit', $template->id) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-zinc-700">Редактировать</a>
                        <button onclick="
                            if (!confirm('Вы уверены что хотите удалить?')) return;
                            fetch('{{ route('admin.document-templates.destroy', $template->id) }}', {
                                method: 'POST',
                        ...
                        " class="w-full text-left px-4 py-2 text-sm text-red-600 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-zinc-700">
                            Удалить
                        </button>
                    </div>
                </div>
