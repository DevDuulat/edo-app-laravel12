...
>
    <input
            type="checkbox"
            class="absolute top-1 left-1 w-4 h-4 accent-blue-500 rounded cursor-pointer z-30"
            @click.stop="selectedFolders.includes({{ $folder->id }}) ? selectedFolders = selectedFolders.filter(id => id !== {{ $folder->id }}) : selectedFolders.push({{ $folder->id }})"
